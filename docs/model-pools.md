...
- 韩文残留修正/译文复核复用对应模式的模型池和同一个 `modelSessionId` 随机顺序；复核时会把对应韩文原文、译文 chunk、术语表和备选翻译段落标记传给模型，用于最小范围修正韩文残留、逻辑明显奇怪的术语误译，以及自动/谷歌翻译常见的人称错误。如果当前模型返回非额度类 400 错误，会继续尝试本次随机顺序里的备用模型，避免因单个模型不支持当前消息格式或参数而直接失败。
- 如果 API 报错被识别为额度/余额/计费相关错误，后端会把当前模型标记为已耗尽，并按本次随机顺序切换到同一模型池里的备用模型重试。
- 如果术语提取把候选高频词和上下文节选交给标准模型后触发敏感内容/内容安全错误，后端会立即改用 `SENSITIVE_FALLBACK_MODELS` 重新提取，并在响应中标记 `sensitiveFallback` 与 `termExtractionFallbackReason`。
