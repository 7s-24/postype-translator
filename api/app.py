...
def dashscope_stream_timeout_kwargs(deadline=None):
    """Build HTTP timeouts for streaming calls.

    The in-loop first/inter-token checks only run after ``next(response)``
    returns a stream event. If the iterator blocks before yielding anything,
    those checks cannot fire, so the HTTP read timeout must also be clipped to
    the first-token budget.
    """
    remaining = _ensure_deadline_budget(deadline)

    read_timeout = min(DASHSCOPE_READ_TIMEOUT_SEC, STREAM_FIRST_TOKEN_TIMEOUT_SEC)
    connect_timeout = DASHSCOPE_CONNECT_TIMEOUT_SEC
    write_timeout = DASHSCOPE_WRITE_TIMEOUT_SEC
    pool_timeout = DASHSCOPE_POOL_TIMEOUT_SEC

    if remaining is not None:
        read_timeout = min(read_timeout, remaining)
        connect_timeout = min(connect_timeout, remaining)
        write_timeout = min(write_timeout, remaining)
...
def translate_chunk_stream(
    ...
    if is_qwen_mt_model(model):
        ...
        response = client.chat.completions.create(
            ...
            stream=True,
            **mt_kwargs,
            **dashscope_stream_timeout_kwargs(deadline),
        )
    else:
        user_prompt = build_translation_user_prompt(
            chunk, index, total, previous_translation, glossary,
